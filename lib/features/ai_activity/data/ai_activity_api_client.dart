...
abstract interface class AiActivityGateway {
...
class AiActivityApiClient implements AiActivityGateway {
  ...
  AiActivityApiClient({
    ...
    Future<void> Function(Duration)? delay,
       ...
       _endpoint = Uri.parse(baseUrl).resolve('/api/v1/analyze/activity'),
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300) {
          try {
            return AiActivityAnalysis.fromJson(
              jsonDecode(response.body) as Map<String, dynamic>,
            );
          } on Object {
            throw const AiApiException(
              AiApiFailureKind.invalidResponse,
              'The AI service returned an unreadable workout.',
            );
          }
        }
        ...
        lastFailure = failure;
        if (!_retryable(failure) || attempt == maxAttempts) throw failure;
        await _delay(
        ...
        lastFailure = const AiApiException(
          ...
          'The AI service took too long to parse the workout.',
        );
      } on SocketException {
        lastFailure = const AiApiException(
          AiApiFailureKind.network,
    ...
    switch (response.statusCode) {
      ...
      case 403:
        return AiApiException(
          ...
          message ?? 'The AI access token is missing or invalid.',
        );
      case 422:
        return const AiApiException(
          AiApiFailureKind.validation,
          'Describe at least one strength exercise and try again.',
        );
      case 429:
  ...
  bool _retryable(AiApiException failure) =>
